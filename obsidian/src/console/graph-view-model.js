...
function buildRunListModel(runs, error, waitingOnly = false) {
  // G4：列表「只看等待人工」过滤——只保留有待决闸门的 run；其余统计不动。
  const visible = waitingOnly
    ? (runs || []).filter(
        (run) =>
          (Array.isArray(run.pending_human) && run.pending_human.length > 0) ||
          run.status === "human_wait"
      )
    : runs || [];
  return {
...
function buildRunDetailModel(detail, pathData) {
  ...
  // G9：文本视图引入 Pilot/Research 语境（与 canvas 侧栏同源判断，
  // graph-view.js:313-323）——approve_call/approve_synthesis 只按 graph_id + 决定集判定。
  const isPilotRun = graphId === "fragment-pilot-v1";
  // rev7：Research run 识别 v1 与宏 v3（创建链 rev6 起新 run 为 v3）。
  const isResearchRun = graphId === "fragment-research-escalation-v1" ||
    graphId === "fragment-research-macro-v3";
  const humanTasks = Object.entries(detail.human_gates)
    .filter(([, gate]) => gate.status === "pending")
    .map(([nodeId, gate]) => {
      const allowedDecisions = [...(gate.allowed_decisions || [])];
      // rev7 P0-4：治理待办分类——等待模型授权（approve_synthesis）与
      // 等待候选决定（accept_result）是两类不同人工闸门；完成态由
      // gate.decision（accept_result/reject）在 node detail 投影区分。
      const taskKind = allowedDecisions.includes("approve_synthesis")
        ? "model_authorization"
        : allowedDecisions.includes("accept_result")
          ? "candidate_review"
          : "other";
      return {
        nodeId,
        ...
        taskKind,
        // G9：pre-gate 语境（Pilot 首闸 / Research 合成闸）——无授权上下文时
        // 渲染提示而非必然失败的按钮。
        isPreGate: isPilotRun && allowedDecisions.includes("approve_call"),
        isResearchPreGate: isResearchRun && allowedDecisions.includes("approve_synthesis"),
      };
    });
  ...
  return {
    runId: run.run_id,
    graphId: run.graph_id,
    // G9：文本视图语境标记（pilot/research 授权闸门识别）。
    isPilotRun,
    isResearchRun,
    status: run.status,
    statusLabel: runStatusLabel(run.status),
...
function renderRunList(container, model, handlers) {
  ...
  // G4：列表「只看等待人工」过滤切换（onToggleWaiting 存在才渲染）。
  if (handlers.onToggleWaiting) {
    const toggle = container.createEl("button", {
      cls: model.waitingOnly
        ? "graph-view-waiting-filter is-on"
        : "graph-view-waiting-filter",
    });
    toggle.setAttribute("type", "button");
    toggle.setText(model.waitingOnly ? "显示全部" : "只看等待人工");
    toggle.addEventListener("click", () => handlers.onToggleWaiting());
  }
  ...
  if (model.humanTasks.length) {
    ...
    for (const task of model.humanTasks) {
      ...
      for (const decision of task.allowedDecisions) {
        // G9：Pilot 首闸 / Research 合成闸在文本回退视图（无画布授权投影）下
        // 不渲染必然失败的 approve 按钮——给可见说明 + 刷新指引。
        if (
          (decision === "approve_call" && task.isPreGate) ||
          (decision === "approve_synthesis" && task.isResearchPreGate)
        ) {
          actions.createEl("p", {
            cls: "graph-view-error",
            text: "该闸门需要授权上下文（画布投影未加载）：请刷新以加载授权材料后再处理。",
          });
          if (handlers.onRefreshRun) {
            const refresh = actions.createEl("button", { cls: "graph-view-refresh" });
            refresh.setAttribute("type", "button");
            refresh.setText("刷新");
            refresh.addEventListener("click", () => handlers.onRefreshRun());
          }
          continue;
        }
...
function renderNodeDetail(container, model, handlers) {
  ...
  if (model.agent) {
    ...
    if (model.agent.unknownSend) {
      // G1-16 冻结文案：发送状态未知，等待人工。绝不出现重试入口或
      // exactly-once 承诺；与后端 projection UNKNOWN_SEND_LABEL 一致。
      el(agent, "p", "graph-agent-unknown", "发送状态未知，等待人工");
    }
  }
